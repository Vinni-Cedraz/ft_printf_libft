...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/27 20:22:38 by vcedraz-          #+#    #+#             */
/*   Updated: 2022/10/27 20:22:41 by vcedraz-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//- converts an upper -
//   case letter to the corresponding lower - case int ft_tolower(int c);

// BYTE MANIPULATION FUNCTIONS:

// memset is the building block of the b_zero and calloc functions but it
// can write any unsigned char, (not only zero), given to it by an ascii
// number:
void				*ft_memset(void *b, int c, size_t len);
// ft_memcpy is like a strncpy but for bytes, it is the callers
// responsability to assure the memory areas do not overlap:
void				*ft_memcpy(void *dst, const void *src, size_t n);
// ft_memmove - copies n bytes from memory area src to memory area dst and
// it does not matter if the memory areas overlap, the function handles it.
void				*ft_memmove(void *dst, const void *src, size_t len);
// ft_memchr searches for a c byte typecasted as an u_char on the first n
// chars of the string s:
void				*ft_memchr(const void *s, int c, size_t n);
// ft_memcmp compares n bytes of s1 and s2 and returns 0 if they are all
// equal or an int > 0 or int < 0 if s1 is smaller or bigger than s2,
// respectivelly:
int					ft_memcmp(const void *s1, const void *s2, size_t n);
// fills a memory area with n null bytes and returns nothing, the void area
// can be typecasted to any type of pointer:
...
// ft_strlcpy copies n bytes from the null terminated string src to dst, but
// at most size - 1 bytes, null terminating the result. It returns the
// length of src, so if the return value is >= size, the output string has
// been truncated.
size_t				ft_strlcpy(char *dst, const char *src, size_t size);
// ft_strlcat concatenates the null terminated src to the null terminated
// dst, but at most size - ft_strlen(dst) - 1,
...
// ft_strnstr searches len bytes for a substring in a string. If little is
// an empty string, big is returned; if little occurs nowhere in big, NULL
// is returned; otherwiwise a pointer to the first character of little is
// returned
...
// ft_strdup duplicates a string and returns a the address to the newly
// created and allocated string (the string it receives doesn't have to
// pre-exist, be allocated or initializied, such that -> ft_strdup("hello")
// is a valid call.
...
// ft_substr searches thru len chars looking for the s[start] element and
// then creates a string that begins there and ends at the usual end of s.
// It works even if len is bigger than the new string's length, but if start
// is bigger than it should, it returns the adress to a null string:
...
// ft_strmapi does the same as ft_striteri but it returns a new string
// instead of modifying the existing one. The other difference is that it
// uses memmory allocation to create the new string and return its address:
char				*ft_strmapi(char const *s, char (*f)(unsigned int, char));
// ft_strtrim receives a string and a set of chars, and it returns a new
// string after every char present in string set from the beggining and end
// of the string s:
...
// ft_putchar writes a char to stdout and returns how many char were written:
...
// ft_itoa_base is a function that converts an int to a string in a
// given base:
